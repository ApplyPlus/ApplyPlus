...
def context_changes(sub_patch):
    ...
    for context_diff in diff_file_patch.context_diffs:
        if context_diff.match_ratio < match.LEVENSHTEIN_RATIO:
            output_message += (f"Context Diff #{context_diff_count}: The match ratio between the source and patch code was low. "
                    ...
                    f" greater than or equal to {match.LEVENSHTEIN_RATIO}. The patch line was {context_diff.patch_line}"
                    f" whereas the file line was {context_diff.file_line}\n")
                
            apply_patch &= CONTEXT_DECISION.DONT_RUN.value

        elif re.search('^(\w+( )?){2,}\([^!@#$+%^]+?\)', context_diff.file_line):
            output_message += (f"Context Diff #{context_diff_count}: We noticed a context-related difference between the function"
        ...
        for line_diff in line_patch_diffs:
            if line_diff[0] == 0 and line_diff[1].rstrip().endswith("="):
                ...
                if function_name:
                    ...
                    output_message += f"Context Diff #{context_diff_count}: R-Value change, User should consider the following\n"
                            f" slicer_output: {var_information})")
                    apply_patch &= CONTEXT_DECISION.RUN.value
                    
                else:
                    output_message += (f"Context Diff #{context_diff_count}: R-Value change but function name was not supplied by"
    ...
    return context_result = ContextResult(apply_patch, output_message)
